...
class Atmosphere {
  ...
  /**
   * Fill the given array with the solar radiotion value in Mj
   */
   ...
   */
  density (densitymap, pressuremap, heatmap) {
    ...
    for (let i = 0; i < densitymap.length; i++) {
      ...
      let pressure = helpers.denormalize(pressuremap[i], this.minPress, this.maxPress) * 1000; // convert to kPa from Pa
   ...
   */
  humidity (humiditymap, heightmap, heatmap, solarmap, pressuremap, windmap) {
  ...
  */
  ...
  saturationPressure2 (temp) {
    return Math.pow(0.6108, ((17.27 * temp) / (temp + 237.3)));
  }
  ...
  saturationDensity (temp, press) {
    ...
    let idealGasConstant = 8.3144598;
    return ((press * molarMassWater) / (idealGasConstant * temp) * 0.001);
  }
