...
class Grid {
  ...
  generate () {
    ...
    */
    ...
    // normalize
    helpers.normalize(this.heightmap);
    // flatten
    ...
    let pressuremap = this.atmo.pressure(heightmap);
    pressuremap = helpers.normalize(pressuremap);
    this.fromArray(pressuremap, 'pressure');

    // calculate initial density
    let densitymap = this.atmo.density(pressuremap, heatmap);
    densitymap = helpers.normalize(densitymap);
    this.fromArray(densitymap, 'density');
  ...
  step (deltaT) {
    ...
    let density = this.atmo.diffuse(this.toArray('density'), 0.1, deltaT);
    this.fromArray(density, 'density');
  }
