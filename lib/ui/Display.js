...
class Display {
   ...
   */
  render (canvas, grid, width, height, options) {
    options         = options || {};
    ...
    else if (renderStyle === 'monochrome') {
      renderFn = this.monochromeBrush.bind(this);
    }
    ...
    for (let i = 0; i < grid.length; i++) {
      let color = renderFn(grid[i]);
      this.setPixel(imgDD, i, color);
    }
  ...
  monochromeBrush (value) {
    let scaled = Math.floor(value * 255);
