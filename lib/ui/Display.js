...
class Display {
  ...
  gradBrush (key, value) {
    ...
    let swidth = 255 / this.stops[key].length;
    let offset = index * swidth;
    let red = stop.R;
    let green = stop.G;
    let blue = stop.B;

    if (red === undefined) {
      let change = Math.floor(Math.min((Math.abs(stop.r) * value), 1) * 255) - offset;
      red = stop.r > 0 ? change : 255 - change;
    }
    if (green === undefined) {
      let change = Math.floor(Math.min((Math.abs(stop.g) * value), 1) * 255) - offset;
      green = stop.g > 0 ? change : 255 - change;
    }
    if (blue === undefined) {
      let change = Math.floor(Math.min((Math.abs(stop.b) * value), 1) * 255) - offset;
