...
class Menu {
  ...
  textMenu (text) {
    if (text.match(/^names/i)) {
      for (let i = 5; i < 10; i++) {
        this.generateNames(5, i).forEach((name) => {
          console.log(`${name}`);
        });
      }
    }
  ...
  gridMenu (text) {
    ...
    // render
      ...
      let height = this.grid.toArray('height');
      this.display.render(height, 'heightmap.png', {renderStyle: 'palette', renderKey: 'terrain'});

      // render the heatmap
      let heat = this.grid.toArray('temperature');
      this.display.render(heat, 'heatmap.png', {renderStyle: 'gradient', renderKey: 'temp'});

      // render pressure map
      let pressure = this.grid.toArray('pressure');
      this.display.render(pressure, 'pressuremap.png', {renderStyle: 'hsl'});
