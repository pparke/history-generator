...
class Myth {
  ...
  write () {
    ...
    this.text =  `${beginning} ${this.nameGen.male().medium().generate()} the
                  ${this.random.pick(adjectives)} ${this.random.pick(creators)},
                  ${this.random.pick(titles)} of ${realm} ${this.random.pick(verbs.past)}
  ...
  /**
   * Returns an array of individuals who make up a direct lineage
   * @param {number} years - the time span over which to operate
   */
  geneology (years) {
    let lineage = [];
    let span = 0;
  ...
  /**
   * Generate an individual with associated sex, name, years lived,
   * number of children
   * @return {object} an object containing the attributes of the individual
   */
  progenitor () {
    let prog = {
